...
bot.onText(//start/,function(m){const id=m.from.id.toString(),d=gdb();bot.sendMessage(m.chat.id,d.users[id]?"Email: "+d.users[id].email:"Ouvre la Mini App pour choisir ton adresse.",{reply_markup:{inline_keyboard:[[{text:"Ouvrir",web_app:{url:RURL+"/miniapp"}}]]}});});
...
app.post("/set-email",function(q,r){const id=q.body.userId,name=q.body.name;if(!id||!name)return r.status(400).json({error:"manquant"});if(!/^[a-z0-9._-]{3,30}$/.test(name))return r.status(400).json({error:"Nom invalide"});const d=gdb();if(d.users[id])return r.status(400).json({error:"deja defini"});if(Object.values(d.users).some(function(u){return u.email===name+"@"+DOM;}))return r.status(400).json({error:"deja pris"});d.users[id]={email:name+"@"+DOM};sdb(d);r.json({email:d.users[id].email});});
app.get("/inbox",function(q,r){const id=q.query.userId,d=gdb();if(!d.users[id])return r.json({emails:[]});r.json({emails:(d.emails||[]).filter(function(e){return e.to===d.users[id].email&&e.expiresAt>Date.now();}).sort(function(a,b){return b.date-a.date;}).slice(0,50)});});
app.post("/receive-email",function(q,r){const d=gdb(),em={id:Date.now()+"-"+Math.random().toString(36).slice(2),to:(q.body.to||"").toLowerCase(),from:q.body.from||"",subject:q.body.subject||"",body:q.body.body||"",date:Date.now(),expiresAt:Date.now()+TTL*60*1000};d.emails.push(em);sdb(d);r.json({success:true});});
